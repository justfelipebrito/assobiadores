...
import { finalizeQualifierMatch } from '../../../../../server/qualifier-finalization-service';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await finalizeQualifierMatch(getAdminFirestore(), {
    ...
    if (!(error instanceof ApiError)) {
      console.error('Qualifier finalization error:', error);
    }
    const response = getErrorResponse(error, 'Erro ao finalizar confronto.');
