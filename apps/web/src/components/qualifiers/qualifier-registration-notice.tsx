...
function QualifierRegistrationNoticeContent({ userId }: { userId: string }) {
  ...
  return (
    <div className="fixed bottom-4 left-4 right-4 z-[80] mx-auto max-w-md rounded-2xl border border-brand-500/30 bg-surface-900/95 p-4 shadow-elevated backdrop-blur-xl sm:left-auto sm:right-6 sm:mx-0">
      <div className="flex gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 text-sm text-surface-400">
            Inscrição por categoria: R$ 4,00. Vencedores avançam para os Regionais.
          </p>
          <Link
            href="/classificatorias"
            className="mt-3 inline-flex h-9 items-center gap-2 rounded-xl bg-brand-500 px-3 text-sm font-semibold text-white shadow-glow-sm transition-colors hover:bg-brand-600"
          >
            Ver regras e inscrição
