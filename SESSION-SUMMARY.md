...
- Mercado Pago local QA setup:
  ...
  - configured production Secret Manager values for the GA4 admin analytics reader and enabled the Google Analytics Data API for project `assobiadores-3f0f6`. A live Data API read against property `535113543` now succeeds with the service account. Remaining GA setup: register the event-scoped `partner_ref` custom dimension in GA4 before the dashboard can group visitors by referral code.
