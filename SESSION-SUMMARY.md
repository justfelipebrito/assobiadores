...
- Mercado Pago local QA setup:
  ...
  - adjusted the public header ticker: it is sticky only on desktop, scrolls away on mobile, and logged-in users now see qualifiers from their `birthState` first; same-date items then sort by participant count before title. Tightened the past daily-highlight winners query to fetch only finalized top-3 rows.
  - refined the public header layout so the brand/menu bar remains sticky on all viewports, while the games/upcoming-events ticker is sticky on desktop and scrolls away on mobile.
  - restored the audio/media player inside the homepage `Destaques Diários` cards while keeping the CTA as a separate link, so the card player controls remain usable.
  - restyled the compact audio player variant to better fit the new daily-highlight card style with a translucent surface, quieter waveform, compact category chip, and clearer integrated play control.
...
- Added admin-only private user profile summaries: the admin `Usuários` page now shows each user's Pix key and warnings when CPF or phone are missing. The UI fetches only minimal private data through a trusted admin API (`pixKey`, `hasCpf`, `hasPhone`) instead of reading `userPrivate` directly from the client.
- Updated the public `/classificatorias` page so anonymous visitors no longer see the top `Classificatórias disponíveis` discovery rail before account actions. Logged-out users now reach the `Inscrição` section first, while logged-in users still see the available categories for their qualifier context. Added helper/page regression tests for the visibility gate.
- Added a production-safe qualifier track seeder for missing-only creation and used it on project `assobiadores-3f0f6`: it created the 72 missing `qualifierTracks` for all Brazilian states x 3 official categories and left the 9 existing production tracks untouched. Verification dry-run after seeding reported 81 existing tracks and 0 missing. The seeded schedule for newly created tracks is `registrationDeadline=2026-06-01T02:59:59Z`, `bracketStart=2026-06-01T03:00:00Z`, and `bracketEnd=2026-07-13T02:59:59Z`.
- Fixed the mobile `Destaques Diários` compact audio layout: the shared audio player now uses fixed compact artwork/control columns and a bounded waveform so the play button remains tappable on narrow screens. The `/destaques` vote cards now use a tighter fixed rank column and stable media/vote heights to reduce clunky mobile layout shifts.
- Added admin qualifier schedule editing and then refactored it to match the product model: the admin Classificatórias page now has one global calendar control for entry deadline, submission start, and submission end across all qualifier tracks that feed Regionais, plus a compact overview table instead of repeated per-track schedule cards. Track rows now use an in-admin `Gerenciar` CTA that opens a detail modal with schedule, stats, generated matches, and track-specific operations instead of sending admins to the public webapp. Updates go through the trusted `/api/admin/qualifiers/schedule` route, which supports both global and single-track schedule updates, revalidates admin role/date ordering server-side, updates `qualifierTracks`, and reschedules editable qualifier matches. Bracket generation/advancement now reads the saved track `bracketStart` instead of only using the hard-coded 2026 fallback.
- Reduced Firestore realtime listener usage after the cost audit: added shared `useDocumentOnce` and `useCollectionOnce` hooks backed by `getDoc/getDocs`, switched broad public overview surfaces (homepage non-live rails, rankings, agenda, battles, championships, qualifiers) and broad admin lists/dashboard tables to one-time reads, and kept realtime deliberately for the homepage header ticker, homepage `Destaques Diários`, active voting/detail flows, moderation, and account/profile surfaces. Admin mutation-heavy pages now refresh one-time data after trusted actions where needed. Added a source-level policy test that guards the retained homepage realtime exceptions and the one-time read policy for broad list pages.
- Fixed stuck Battle lifecycle states in Cloud Functions: the scheduled battle watcher now runs every 5 minutes, moves due battles through `draft -> registration`, `registration -> active`, `active -> voting` using `votingStart` instead of `submissionDeadline`, and finalizes due `voting` battles through the same trusted finalization handler after `votingEnd`. Finalization now safely closes voting battles with no approved submissions as `finished` with no winners/no scoring, instead of throwing and leaving them stuck. Added lifecycle tests for the date boundaries, scheduled finalization, and no-submission finalization, and added the missing `status + registrationEnd/votingStart/votingEnd` Firestore indexes required by the watcher queries.
- Fixed production admin Battle manual operations: the `/api/battles/{battleId}/tiebreak` web route now supports CORS preflight/response headers for the Admin App Hosting origin, and the web admin finalization service now mirrors the Cloud Function no-winner behavior by finishing battles with no approved or no confirmed-participant submissions without awarding points. Added route CORS tests and finalization edge-case tests, then verified the web production build.
- Fixed the Battle finalization 500 root cause: both the web admin finalizer and scheduled Cloud Function finalizer no longer add an unnecessary `orderBy('voteCount')` to the approved-submissions query, so finalization follows the product rule directly: after `votingEnd`, finish the battle when the top public-vote result is not tied; require tiebreak only when the top result is tied. Added tests that guard against reintroducing the index-dependent `orderBy`, and added extra battle finalization indexes for remaining trusted equality queries.
- Fixed the public Ranking page data source regression: `Geral` now reads the all-time `users` leaderboard again, while `Temporada` uses the active season `seasonRankings/{seasonId}/users` read model only when explicitly selected. Added a regression test so the ranking page cannot become season-only again. The AdSense/service-worker `pagead2.googlesyndication.com` console error is expected when ads are blocked and is not the ranking data source.
